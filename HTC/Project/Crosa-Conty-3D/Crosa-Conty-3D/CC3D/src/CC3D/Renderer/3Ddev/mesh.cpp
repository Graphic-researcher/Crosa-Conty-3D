...
namespace CC3D
{
		...
		:m_Type(type)
	{
		...
		case CC3D::MeshType::Plane:
			CreatePlane(sample);
			break;
		case CC3D::MeshType::Cube:
			CreateCube(sample);
			break;
		case CC3D::MeshType::Sphere:
			CreateSphere(sample);
			break;
		case CC3D::MeshType::Model:
			CC3D_CORE_ASSERT(false, "MeshType::Model can't init by this method!");
			return;
		default:
			break;
		}
	...
	void TriMesh::ObjLoader(const std::string& path)
	{
		...
		{
			CC3D_CORE_ASSERT(false,"Couldn't load .obj model!");
