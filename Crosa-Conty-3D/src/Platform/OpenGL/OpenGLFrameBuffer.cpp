...
namespace CC3D {
	...
	namespace Utils {
		...
		{
			glCreateTextures(TextureTarget(multisampled), count, outID);// Create mutiple textures
		}
		...
		static void AttachColorTexture(uint32_t id, int samples, GLenum internalFormat, GLenum format, uint32_t width, uint32_t height, int index)
		{
			...
			if (multisampled)
			{
				glTexImage2DMultisample(GL_TEXTURE_2D_MULTISAMPLE, samples, internalFormat, width, height, GL_FALSE);//format 是渲染的纹理种类 https://www.khronos.org/registry/OpenGL-Refpages/gl4/html/glTexImage2DMultisample.xhtml
			}
			else
			{
	...
	void OpenGLFramebuffer::Invalidate()
	{
		...
		{
			CC3D_CORE_ASSERT(m_ColorAttachments.size() <= 4, "m_ColorAttachments.size() Biger than 4");
